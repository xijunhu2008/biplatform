    ...
    */
    ...
    this.styleHandler = {
        ...
        get: function (data) {
            ...
            *标签类型 文本= 0 日期=1 数值 = 2 
            */
            chartTemplet = this.xAxisStyle(chartTemplet, data);

            /*
            *4.配置Y轴样式
            */
            chartTemplet = this.yAxisStyle(chartTemplet, data);

            return chartTemplet;
        },
        AxisStyle: function (chartTemplet, Axis, config, data) {
            ...
            if (Axis == "xAxis") {
                ...
                if (data.Rows.length > 1000) {
                    //设置跳数
                    chartTemplet[Axis].tickInterval = labelstep;
                } else {
                    //如果大于十条数据就跳过某些label 免得x轴拥挤
                    if (data.Rows.length != labelstep) {
                        //将跳数应用于labels.step之上
                        //廖力修改于2013/9/17
                        chartTemplet[Axis].labels.step = labelstep;
                        //设置一下rotation将这些lable压至一行，否则labels们就会呈阶梯状,让人费解
                        chartTemplet[Axis].labels.rotation = 0.1;
                    }
                }
            }
...
//                chartTemplet[Axis].title = {
//                    align:''
//                }
//            }

            //设置格式和旋转角度
            var chartTemplet = (function (chartTemplet) {
                var _thatChart = self;


                //解决格式问题
                chartTemplet[Axis].labels.formatter = function () {
                    //给加上‘前缀’，‘后缀’的方法
                    var priPostFac = function (str) {
                        return $.trim(config.Postfix) + str + $.trim(config.Prifix);
                    }
                    //标签类型 文本= 0 日期=1 数值 = 2 
                    switch (config.Type) {
                        case '0': //文本
                        ...
                        case '1': //日期
                            ...
                            if (config.Format != '') {
                                return priPostFac(formatDate(val, config.Format));
                            } else {
                                return priPostFac(this.value);
                            }
                        case '2': //数字
                            ...
                            //中文
                            var lan = 'none';
                            if (config.NumberConvertCH) {
                                lan = 'ch';
                            } //英文
                            else if (config.NumberConvertEN) {
                                lan = 'en';
                            }
                            tempValue = formatBigNumber(tempValue, lan);
                            //2.小数位
                            if (Number(config.Decimal) != 0) {
                                tempValue = Number(tempValue).toFixed(config.Decimal);
                            }

                            return priPostFac(tempValue);
                    }
                }

                //旋转角度
                if (config.Rotation) {
                    chartTemplet[Axis].labels.align = 'left';
                    chartTemplet[Axis].labels.rotation = config.Rotation;
                }
                return chartTemplet;
            }).call(self, chartTemplet);
    ...
    this.templet = {
        //x轴项目
        xAxis: { name: "", eprChartMapping: "", data: [], showInLegend: true, type: "", yAxis: 0, index: 0, showLabel: false },
        //带label的图
        lableData: {
            dataLabels: {
                //此处勿轻易配置，
