...
jmreport.reportControls.controlTypes['gridViewData'] = function (config) {
    ...
    */
    this.bind = function (data, pageIndex) {
        ...
        if (this.data.Tables[0].Rows.length > 500 && !this.config.config.isServerPager && Number(this.config.pagesize) == 0) {
        ...
        //判断用户是否想要总汇
        var columnsCondition = this.combins.findIsAnyCombie(this.config.config,this.data.Tables[0]);
        if (columnsCondition.length != 0) {
            //开始计算总汇数据 并在表格的最后一行加入总汇数据
            dt.Rows = dt.Rows.concat(this.combins.init(columnsCondition, this.data.Tables[0]));
        }
    ...
    */
    this.createTable = function (dt, columns) {
        ...
        if (this.data.Tables[0].Rows.length >= 500) {
            head.append("<tr class='jmreport-max-data-alt' ><th colspan='" + dt.Columns.length + "' > 查询耗时(ms)：" + dt.SearchTime + ",注意，此次查询数据量可能已超过五百条，数据将折页显示，若要查看完整数据，请点击表格右上方导出。 </th></tr>");
        }
        ...
        var bigData = function () {
            ...
            var loadDataByStep = function () {
                var tempInterval = setInterval(function () {
                    ...
                    //判断结束下标是否超出数组范围
                    if (dt.Rows[endIndex]) {
                        tempData.Rows = dt.Rows.slice(startIndex, endIndex);
                    } else {
                        //如果超出数组范围就使用最后一次设置的开始位置，到数组的最后一个index取到最后一组数据
                        tempData.Rows = dt.Rows.slice(startIndex, dt.Rows.length);
                        isEnd = true;
                    }
                    ...
                    if (thisObj.selecter.pageSize != 0) {
                        ...
                        rowLength = tempData.Rows.length;
                    }
                            ...
                            if (item.Value) {
                                ...
                                if (column['isDisplayPre'] || item.Value.indexOf('%') != -1) {
                                    ...
                                    if (!isNaN(tempValue)) {
                                        ...
                                        agentDiv.append("<div class='preValue' style='" + fontColor + "' >" + item.Value + "</div>");
    ...
    this.combins = {
            ...
            if (config.ControlConfig && config.ControlConfig.gathers && config.ControlConfig.gathers.length) {
                for (var i = 0; i < table.Columns.length; i++) {
                    ...
                    $(config.ControlConfig.gathers).each(function (i, g) {
                        if (column.Name == g.column || column.OldName == g.column) columnsCondition.push({ columnName: column.Name, combinType: g.fun });
                    });                                       
                }
        ...
        cleateMasterRow: function (row) {
            ...
            newRow.setValue = function (name, value, combinType) {
                ...
                if (!item) {
                    this.ItemArray.push(item={ ColumnName: name, Value: value });
                }
                //填上ItemArray对应下标的value
                item.Value = value.toString();
                //告诉设置一个combinType值以表示该字段是以什么类型的总汇得出结果的
                item.combinType = combinType;
                //再设置外部值
                this[name] = value;
            }
    ...
    */
    this.createGridHeader = function (data) {
        ...
        for (var i = 0; i < data.Columns.length; i++) {
            ...
            if (typeof (item.DisplayName) == 'undefined' || item.DisplayName == '' || item.DisplayName == null || item.DisplayName == "null") {
                columns = addChild(columns, item.Name, data.Columns[i]);
            } else {
