...
jmreport = {
    ...
    */
    ...
    //打开报表页面
    openReportPage: function (pageId, title, toolbar, content, callback) {
    ...
    this.load = function () {
        ...
        jmreport.proxy.page.getById(self.pageId, function (p, ex) {
            ...
            if (p) {
                ...
                message.error(ex.statusText || ex || '加载页面失败');
            }
    ...
    this.toolbar = function (target, config) {
        ...
        */
        ...
        this.render = function () {
            ...
            if (this.config.ToolbarControls && this.config.ToolbarControls.length > 0) {
                this.panels = {};
                for (var i = 0; i < this.config.ToolbarControls.length; i++) {
                    var c = this.config.ToolbarControls[i];
    ...
    this.report = function (target, config) {
        ...
        this.render = function () {
            ...
            */
            if (this.config.Template && this.config.Template.Content) {
                if (typeof this.config.Template.Content == 'string') this.config.Template.Content = JSON.parse(this.config.Template.Content);
        ...
        */
        this.renderControl = function () {
            ...
            if (this.config.Template.Content) {
                for (var i = 0; i < this.config.Template.Content.length; i++) {
                    var row = this.config.Template.Content[i];
                    var tr = $('<tr></tr>');
