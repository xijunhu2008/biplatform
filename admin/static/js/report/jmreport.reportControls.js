...
/*
        ...
        ////在线编辑
        //if (oap.systemOptions.config.controlMenu.moreMenu.onlineDataEdit == 1) {
        //    this.config.createMoreMenu.call(this, moreMenu, {
        //        name: 'downLoadCsv',
        //        title: '在线数据处理',
        //        handler: function () {
        ...
        */
        ...
        ,
            ...
            DataLength: function (data) {
                ...
                //判断是否有行
                else if (!data.Tables[0].Rows || data.Tables[0].Rows.length == 0) {
                    that.log("暂无数据哦，亲！");
                    this.onErrorSystemAction();
                    return true;
                }
        ...
        ,
            ...
            isDataHasError: function (data) {
                if (!data.Message) {
                    ...
                    that.log("不好意思，数据出错啦。" + data.Message);
                    this.onErrorSystemAction();
                    return true;
                }
            }
                ...
                if (data.Tables[0].Rows && data.Tables[0].Rows.length > 0) {
                    for (var i = 0; i < data.Tables.length; i++) {
                        ...
                        dt.getColumnMapping = function (name) {
                            if (dt.Mappings) {
                                for (var j = 0; j < dt.Mappings.length; j++) {
                                    var mapping = dt.Mappings[j];
